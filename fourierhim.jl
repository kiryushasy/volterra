...
# Функции для вычисления параметров f0-f6, k1, k2
function compute_parameters()
    # Значение n от 0.05 до 0.85 с шагом 0.05
    ...
    function solve_k!(F, vars, eta)
        ...
        integrand_k2(x) = -x * (alpha_val(eta) + beta_val(eta) * x + gamma_val(eta) * x^3) * sin(k2 * x) * cosh(k1 * x)
    ...
    function system_f!(F, vars, eta)
        ...
        F[5] = f4 + 12 * eta * (2 * f0 * psi_3(eta) + psi_3(eta) + sum([(-1)^m * f[m+1] * (
                psi_4(eta) / factorial(m + 1) -
                psi_5(eta) / factorial(m + 2)
            ) for m in 0:6]))
        
        F[6] = f5 + 12 * eta * (2 * f1 * psi_3(eta) + psi_4(eta) + sum([(-1)^m * f[m+1] * (
                psi_5(eta) / factorial(m + 1)
            ) for m in 0:6]))
        
        F[7] = f6 + 12 * eta * (2 * f2 * psi_3(eta) + 2 * f0 * psi_5(eta) + psi_5(eta))
    end
...
# вторая производная
function f_double_prime(z::Float64, params::Dict)
    if z ≤ 1.0
        terms = [m * (m-1) * params[Symbol("f$m")] / factorial(m) * (-1)^m * (1 - z)^(m-2) for m in 2:6]
        return sum(terms)
    else
        f0 = params[:f0]
        f1 = params[:f1]
...
function compute_μ2(data)
    ...
    for params in data
        n = params[:n_value]
        f0 = f(0.0, params) #поменять на f в 0
        
        # интеграл числителя: ∫z₁ F(z₁) dz₁
        numerator, _ = quadgk(z -> z * F_function(z, params), 0.03, 100000, rtol=1e-8)
        
        # интеграл знаменателя: ∫f(z₁) dz₁
        denominator_integral, _ = quadgk(z -> z * f_prime(z, params), 0.03, 100000, rtol=1e-8)
        
        denominator = 1 + f0 + 2 * denominator_integral
        μ2 = abs(denominator) < 1e-10 ? 0.0 : 4π * n * (2 * numerator) / denominator
        push!(μ2_values, μ2)
    end
    return μ2_values
end
...
function compute_pressure(data, μ_total, n_values)
    # Создаем сплайн для μ(n) для более гладкого интегрирования
    μ_spline = Spline1D(n_values, μ_total; k=3)
    
    pressure_values = Float64[]
    for (i, n) in enumerate(n_values)
        # Первое слагаемое: n*μ(n)
        term1 = n * μ_total[i]
        
        # Второе слагаемое: интеграл от μ(ñ)dñ от 0.05 до n
        term2 = quadgk(x -> μ_spline(x), 0.05, n, rtol=1e-8)[1]
